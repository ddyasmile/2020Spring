Shader "Unlit/MyBRDFShader"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float tmp = pow(1 - cosA, 5);
                float3 ret = R + (1 - R) * tmp;
                return ret;
            }
            ...
            float GGX_D(float roughness, float NdotH)
            {
                ...
                float ret = roughness2/(UNITY_PI * pow(NdotH2 * (roughness2 - 1) + 1, 2));
                return ret;
            }
 
            float CookTorrence_G (float NdotL, float NdotV, float VdotH, float NdotH){
                float tmp1 = (2 * NdotH * NdotV) / VdotH;
                float tmp2 = (2 * NdotH * NdotL) / VdotH;
                return min(tmp2, min(1, tmp1));
            }
